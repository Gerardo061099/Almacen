...
$(document).ready(function () {
    ...
    $('#frm_add_user').submit(function (e) { 
        ...
        datos = JSON.stringify({'registro_id':registro_id,'nombre':nombre,'apellidos':apellidos,'email':email,'pass':pass,'num_e':num_e,'estado':estado,'opcion':option})
        if (nombre != '' && apellidos != '' && email != '' &&  num_e != '' && estado != 'Choose...') {
            $.ajax({
                type: "POST",
                url: "php/dataUsuarios.php",
                data: datos,
                ...
                success: function (a) {
                    console.log(a)
                    if (a.result == '1') messageSWA('El usuario se agrego correctamente!!','success')
                    if (a.result == '') messageSWA('Ocurrio un error inesperado','error')
    ...
    $(document).on('click','.btnBorrar', function () {
        ...
        let response = confirm(`Seguro que quieres eliminar el siguiente registro: ${delete_row}`)
        if (response) {
            $.ajax({
                type: "POST",
                url: "php/dataUsuarios.php",
                ...
                success: function (resp) {
                    if (resp.result == '1') messageSWA('El registro se elimino','success')
                    if (resp.result == '') messageSWA('Ocurrio un error al intentar eliminar el registro','error')
                    tabla_usuarios.ajax.reload(null,false)
                }
            });
        } else {
