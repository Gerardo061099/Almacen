...
// Cabecera de página
    function Header() {
        ...
        $this->setXY(70,30);
        $this->Cell(50,15,'Lista de Herramientas Agotadas',0,0,'C',0);
        $this->Ln(20);
    }
...
$pdf->Cell(16,8,'Stock',1,0,'C',1);
$pdf->Cell(20,8,'Adquirir',1,1,'C',1);
$pdf->SetTextColor(1, 1, 1);
$herramientasquery = mysqli_query($conexion,"SELECT h.id_herramienta AS id,h.Nombre AS nombre,c.material AS material,c.descripcion AS descripcion,g.Num_gavilanes AS gavilanes,m.Ancho AS ancho,m.Largo AS largo,h.cantidad_minima AS stock_m,h.cantidad AS stock FROM $tbherr_db7 h inner join categorias c on h.id_categoria = c.id_categoria inner join gavilanes g on h.id_gavilanes = g.id_gav inner join medidas m on h.id_medidas = m.id_medidas  WHERE cantidad < Cantidad_Minima ORDER BY id_herramienta"); 
while ($result_data = mysqli_fetch_array($herramientasquery)) {
    ...
    $pdf->Cell(16,8,$result_data['stock'],1,0,'C',0);
    $pdf->Cell(20,8,$result_data['stock_m']-$result_data['stock']. ' Pieza(s)',1,1,'C',0);
}
$pdf->Output();
?>